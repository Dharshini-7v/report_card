...
const mockStore = (() => {
    ...
    if (!data.students) data.students = []; // array of processed students
    if (!data.summary) data.summary = { classAverage: 0, gradeCounts: {} };
    function recalcSummary() {
        const arr = data.students || [];
        if (arr.length === 0) { data.summary = { classAverage: 0, gradeCounts: {} }; save(data); return; }
        const avg = arr.reduce((s, x) => s + (x.average || 0), 0) / arr.length;
        const gradeCounts = {};
        arr.forEach(s => { gradeCounts[s.grade] = (gradeCounts[s.grade] || 0) + 1; });
        data.summary = { classAverage: avg, gradeCounts };
        save(data);
    }
    return { data, save, recalcSummary };
})();
...
async function apiFetch(endpoint, options = {}) {
    ...
    if (endpoint === '/summary' && method === 'GET') {
        mockStore.recalcSummary();
        return mockResponse(mockStore.data.summary);
    }

    // /students
    if (endpoint === '/students' && method === 'GET') {
        return mockResponse(mockStore.data.students || []);
    }
    ...
    if (endpoint === '/processReport' && method === 'POST') {
        ...
        mockStore.data.students = processed;
        mockStore.recalcSummary();
        return mockResponse({ students: processed, summary: mockStore.data.summary });
    }
